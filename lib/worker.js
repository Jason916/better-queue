...
Worker.prototype.progressBatch = function (complete, total, msg) {
  var self = this;
  if (!self.active) return;
  self.progress.complete = complete != null ? complete : self.progress.complete;
  self.progress.total = total != null ? total : self.progress.total;
  self._eta.done = self.progress.complete;
  self._eta.count = self.progress.total;
  self.progress.eta = self._eta.format('{{etah}}');
  self.progress.pct = Math.max(0, Math.min(1, complete / total));
  self.progress.message = msg != null ? msg : self.progress.message;
...
Worker.prototype.progressTask = function (id, complete, total, msg) {
  ...
  if (!self.active) return;
  if (!self._waiting[id]) return;
  task = self.progress.tasks[id];
  task.complete = complete = complete != null ? complete : task.complete;
  task.total = total = total != null ? total : task.total;
  task.message = msg != null ? msg : task.message;
  task.pct = Math.max(0, Math.min(1, complete / total));
  self.emit('task_progress', id, task);
}
